...
use crate::{
    ...
    metadata_storage::DownloadSession,
};
use egui::{Context, Ui};
...
pub struct IaGetApp {
    ...
    // Progress tracking
    download_session: Option<Arc<Mutex<DownloadSession>>>,
...
impl IaGetApp {
    ...
    fn start_download(&mut self, ctx: &Context) {
        ...
        if !self.recent_downloads.contains(&self.archive_identifier) {
            self.recent_downloads.insert(0, self.archive_identifier.clone());
        ...
        let include_formats: Vec<String> = if include_formats.is_empty() {
            ...
            include_formats.split(',').map(|s| s.trim().to_string()).collect()
        };
            ...
            handle.spawn(async move {
                let _result = controller
                    .start_download(
                        ...
                        output_dir,
                        include_formats,
                        if max_size.is_empty() { None } else { Some(max_size) },
                        dry_run,
                        progress_tx,
                    )
                    .await;
            ...
            ui.horizontal(|ui| {
                if self.is_downloading {
                    ...
                    if self.download_progress > 0.0 {
                        ui.add(egui::ProgressBar::new(self.download_progress / 100.0)
                            .show_percentage());
                    }
                } else {
                    ui.label("Ready");
    ...
    fn render_download_tab(&mut self, ui: &mut Ui, ctx: &Context) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add(egui::Slider::new(&mut self.config.concurrent_downloads, 1..=16));
        });
        ...
        // Download button
        ui.horizontal(|ui| {
            if ui.add_sized([100.0, 30.0], egui::Button::new("Download"))
                .clicked() && !self.is_downloading {
        ...
        if let Some(rx) = &mut self.progress_rx {
            while let Ok(update) = rx.try_recv() {
                ...
                if update.total_files > 0 {
                    self.download_progress = (update.completed_files as f32 / update.total_files as f32) * 100.0;
                }
            }
        }
