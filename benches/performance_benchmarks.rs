...
fn bench_performance_monitoring(c: &mut Criterion) {
    ...
    group.bench_function("record_download", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                monitor.record_download(black_box(1024 * 1024), black_box(Duration::from_millis(100))).await;
            });
        });
    });
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let handles: Vec<_> = (0..5).map(|i| {
                    ...
                    async move {
                        let size = client_clone.get_optimal_buffer_size(
                            Some(black_box((i + 1) * 10 * 1024 * 1024))
                        ).await;
                        black_box(size);
                    }
                }).collect();
